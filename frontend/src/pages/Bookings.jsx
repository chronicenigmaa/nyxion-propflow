...
function AddBookingModal({ onClose, onAdd, clients }) {
  ...
  return (
    <Modal title="Schedule booking" sub="Viewing, signing, or inspection" onClose={onClose}
...
export function BookingsPage() {
  const { bookings, addBooking, updateBooking, clients } = useAppStore();
  ...
  return (
    <div className="p-5">
      ...
      <Card>
        ...
        {filtered.length === 0 && <div className="py-12 text-center text-[13px] text-gray-400">No bookings found.</div>}
        {filtered.map(b => {
          const client  = clients.find(c => c.id === b.clientId);
          const unit    = UNITS.find(u => u.id === b.unitId);
          const project = unit ? PROJECTS.find(p => p.id === unit.projectId) : null;
          return (
            <div key={b.id} className="grid items-center px-4 py-3.5 border-b border-gray-100 hover:bg-gray-50 transition-colors"
              style={{ gridTemplateColumns:"2fr 2fr 1fr 1fr 130px 130px" }}>
              ...
              <select value={b.type} onChange={e => updateBooking(b.id, { type:e.target.value })}
                style={{ height:30, borderRadius:8, border:"1px solid #E5E7EB", padding:"0 8px", fontSize:12, color:"#374151", fontFamily:"inherit", outline:"none", background:"#fff", cursor:"pointer", width:120 }}>
              ...
              <select value={b.status} onChange={e => updateBooking(b.id, { status:e.target.value })}
                style={{ height:30, borderRadius:8, border:"1px solid #E5E7EB", padding:"0 8px", fontSize:12, fontFamily:"inherit", outline:"none", background:"#fff", cursor:"pointer", width:120,
                  color: b.status==="completed"?"#057A55":b.status==="cancelled"?"#E02424":"#1C64F2", fontWeight:500 }}>
      ...
      {showModal && <AddBookingModal onClose={() => setShowModal(false)} onAdd={addBooking} clients={clients} />}
    </div>
  );
}